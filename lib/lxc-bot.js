...
function getProject(containers, id) {
    return _.findKey(containers, function (container) {
        return container === id;
    });
}

function search(containers, pids, pid) {
    var match = _.find(pids, function (elem) {
        var v =  _.any(elem.pids, function (p) {
            return p == pid;
        });
        return v;
    });
    return getProject(containers, match.container);
}
...
function start(opts) {
    ...
    return q.all([
        ...
        if (opts.search) {
            var match = search(containers, pidList, opts.search);
            return console.log('PID %d belongs to %s.', opts.search, match);
        }

        pidList.forEach(function (elem) {
            console.log('User/Project: %s\nContainer: %s', getProject(containers, elem.container), elem.container);
